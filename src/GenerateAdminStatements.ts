...
import { formatDate, total } from './utils';
  ...
  try {
    ...
    const ordersGroupedByRetailer = ordersSnapshot.docs
      ...
      .reduce((result: any, item: any) => {
        (result[item.retailerRef.id] = result[item.retailerRef.id] || []).push(item);
        return result;
      }, {});
    ...
    const customers = customersSnapshot.docs.map((documentSnapshot:any) => {
      const customer = documentSnapshot.data() as Customer;
      customer.id = documentSnapshot.id;
      return customer;
    });

    const retailers = retailersSnapshot.docs.map((documentSnapshot:any) => {
      ...
      if (orders) {
        ...
        for (const day of Object.keys(groupedByDay)) {
          detailedInvoicesData.push({
            ...
            // @ts-ignore
            data: formatAdminDetailedOrdersForReport(groupedByDay[day], customers),
        ...
        const data = await formatAdminOrdersForReport(orders, retailer.id);
        adminData.push({
          retailerRef: retailer.ref,
          name: retailer.name,
    ...
    for (const documentSnapshot of snapshot.docs) {
      await admin.firestore().doc(`temp/${documentSnapshot.id}`).delete().catch((err: any) => console.log(err));
    }

    // Return a single array containing all objects
    return adminData;
  } catch (err) {
    console.error(err);
    throw err;
